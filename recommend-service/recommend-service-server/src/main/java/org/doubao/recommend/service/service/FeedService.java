...
public class FeedService {
    ...
    public FeedResponse getFeed(RecommendRequest request) {
        ...
        List<RecommendItem> paged = filterByCursor(ranked, cursor).stream().limit(request.getPageSize()).collect(Collectors.toList());

        FeedResponse response = new FeedResponse();
        response.setItems(paged);
        response.setHasMore(filterByCursor(ranked, cursor).size() > paged.size());
    ...
    private void normalizeRequest(RecommendRequest request) {
        ...
        if (request.getLimit() == null || request.getLimit() <= 0) {
            request.setLimit(maxCandidates);
        }
    }
    ...
    private Set<CandidateItem> ensureEnoughCandidates(RecommendRequest request, Set<String> seen) {
        ...
        for (RecallStrategy strategy : recallStrategies) {
            ...
            merged.addAll(recalled.stream().limit(recallLimitPerStrategy).collect(Collectors.toList()));
    ...
    public List<RecommendItem> filterByCursor(List<RecommendItem> ranked, CursorInfo cursor) {
        ...
        return ranked.stream().filter(item -> item.getScore() < cursor.getScore()
                        || (Objects.equals(item.getScore(), cursor.getScore()) && item.getContentId() < cursor.getContentId()))
